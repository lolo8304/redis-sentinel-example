...
# Build
# - docker-build: build Nest image locally
# - build-and-upgrade: build then helm upgrade the release
...
# Deployment
# - install: helm install/upgrade (creates namespace if needed)
# - upgrade: helm upgrade + refresh nest ConfigMap
# - uninstall: helm uninstall release
# - rollout: restart redis statefulset (redis cluster only)
# - dry: helm client-side dry run with debug
...
# Debugging (cluster)
# - node[0-2]-info: redis INFO replication per pod
# - bash[0-2]: shell into sentinel containers
# - bashn: shell into Nest pod
...
# Observability
# - events: latest namespace events
# - logs: follow redis containers across pods
...
# Local debugging
# - port: port-forward redis + sentinel services and pods
# - master: show current master from sentinel
# - loop: poll master continuously
...
# Redis HA testing (single operations)
# - write, writeRnd, writeAll: seed data via API
# - read, readRnd: read data via API
# - health: one-off health check endpoint
alias write='curl -XPOST http://localhost:8080/example/user/42 -H "Content-Type: application/json" -d "{\"name\":\"Lolo\"}"'
alias writeRnd='r=$((RANDOM % 30)); echo -n "$(date): "; curl -XPOST http://localhost:8080/example/user/$r -H "Content-Type: application/json" -d "{\"id\":\"$r\",\"name\":\"Lolo-$r\"}";echo ""'
alias writeAll='for i in $(seq 0 29); do curl -XPOST http://localhost:8080/example/user/$i -H "Content-Type: application/json" -d "{\"id\":\"$i\",\"name\":\"Lolo-$i\"}"; echo ""; done'

alias read='curl -XGET -s http://localhost:8080/example/user/42'
alias readRnd='r=$((RANDOM % 30)); echo -n "$(date): "; curl -XGET -s http://localhost:8080/example/user/$r; echo ""'

# Continuous testing loops
# - test-write/test-writeRnd: continuous write traffic
# - test-read/test-readRnd: continuous read traffic
# - test-health: continuous health checks
alias test-read='while true; do echo "$(date): $(read)"; sleep 1; done'
alias test-readRnd='while true; do readRnd; sleep 1; done'
alias test-write='while true; do echo "$(date): $(write)"; sleep 1; done'
